...
from ltr.models.bbreg.atom_SCR_Det import AtomSCRDetMTL
from ltr.external.PreciseRoIPooling.pytorch.prroi_pool import PrRoIPool2D

#IoU-Net Training
import ltr.data.processing_utils as prutils
import ltr.models.loss.kl_regression as klreg_losses
#from ltr.models.bbreg.atom_SCR_Det import IoU_MLP
...
class Shot_Memory():
    ...
    def __init__(self, memory_size):
        super(Shot_Memory, self).__init__()
        self.memory_sz=memory_size
        self.flag_fulled_one_time=False
        self.current_pointer=0
        self.update_pointer=0
        self.num_image=1 #We just consider the original frame (no augs) for shots!

        self.memory_feat=[None for _ in range(self.memory_sz)]
        self.memory_box=[None for _ in range(self.memory_sz)]
        #self.memory_score=torch.zeros(self.memory_sz)#,self.num_image)

    def init_memory(self, init_feats, init_boxes, sample_scores=None,num_aug_used=None):
        #Omit frame 1 and only consider augmentations
        if not num_aug_used:
            num_aug_used=init_feats[0].size(0)-1

        for i in range(num_aug_used):
            self.memory_feat[i] = TensorList([x[i+1:i + 2, ...] for x in init_feats])
            self.memory_box[i] = init_boxes[i+1].cpu()
        self.update_pointer= num_aug_used
        self.current_pointer = num_aug_used  # Subtitute samples in a circle (we can use flag & max_score) for index to substitute


    def update_memory(self, sample_feat, sample_box, sample_score=None):
        if self.current_pointer >= self.memory_sz:
            self.current_pointer = self.update_pointer #Subtitute samples in a circle (we can use flag & max_score) for index to substitute
            self.flag_fulled_one_time = True
        self.memory_feat[self.current_pointer] = sample_feat
        # self.memory_score[self.current_pointer] = sample_score
        self.memory_box[self.current_pointer] = sample_box #sample_box format check!
        self.current_pointer += 1


    def get_shots(self):
        return_feat=self.memory_feat if self.flag_fulled_one_time else self.memory_feat[:self.current_pointer]
        return_box = self.memory_box if self.flag_fulled_one_time else self.memory_box[:self.current_pointer]
        return return_feat,return_box
...
class DiMP(BaseTracker):
    ...
    def initialize_features(self):
        ...
        if not getattr(self, 'features_initialized', False):
            ...
            if self.use_mtl:
                new_bb_regressor = AtomSCRDetMTL(input_dim=(4 * 128, 4 * 256), pred_input_dim= (256, 256),
                                                  pred_inter_dim= (256, 256),pretrained_bbreg=self.params.net.bb_regressor).eval().cuda()
                self.params.net.net.bb_regressor=new_bb_regressor
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.use_mtl = False
        self.use_augs_mtl = False # Whether to use augmentations in memory
        self.frame_num = 1
        ...
        #x= self.net.classifier
        if self.use_mtl:
            #IoU-Net train
            self.proposal_params = {'boxes_per_frame': 128, 'gt_sigma': (0.05, 0.05),
                           'proposal_sigma': [(0.05, 0.05), (0.5, 0.5)]}
            # self.proposal_params = {'boxes_per_frame': 10, 'gt_sigma': (0.3, 0.0005),
            #                         'proposal_sigma': [(0.0005, 0.0005), (0.1, 0.1)]}

            self.iounet_objective=klreg_losses.KLRegression()
            self.iounet_loss_weight = 1  # 0.0025



            for name, param in self.net.bb_regressor.main_network.named_parameters():
                if name.find('mtl') != -1:
            ...
            #     if param.requires_grad:
                    #print(name)
            #print('End Debug****')
            self.iounet_optimizer = optim.Adam(
                [{'params': self.net.bb_regressor.iou_predictor.parameters(), 'lr': 1e-4}, #Pass iou_predictor
                 {'params':filter(lambda p: p.requires_grad, self.net.bb_regressor.main_network.parameters()),'lr' : 1e-3}]) #mtl is true in main_network -> we just pass mtl


            self.backbone_features_memory = Shot_Memory(memory_size=15)
        ...
        if self.use_mtl:
            if self.use_augs_mtl: #Box: First and Last two boxees are Identity (15 boxes in total)->Omit last two when passing!. First feature is Frame 1 (13 feats in total)
                self.backbone_features_memory.init_memory(self.initaug_iou_backbone_feat, self.initaug_target_boxes, num_aug_used=8)
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        self.frame_num += 1
        if self.frame_num==2:
            for n, p in self.net.neck.named_parameters():
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      ...
                                                                      self.img_sample_sz)
        self.backbone_feat=backbone_feat
        ...
        if update_flag and self.params.get('update_classifier', False):
            ...
            if self.use_mtl and (((self.frame_num - 5) % 15 == 0)):# or  and self.max_score1 > 0.7: #and self.flag == 'normal':
                self.net.bb_regressor.eval()
                for n, p in self.net.bb_regressor.named_parameters():
                    if n.find('mtl') != -1 or n.find('iou_predictor') != -1:
                        p.requires_grad = True

                    else:
                        p.requires_grad = False

                self.update_iounet(target_box)
            ...
            if self.use_mtl and (((self.frame_num - 2) % 10 == 0)) and  self.max_score1 > 0.7:
                self.backbone_features_memory.update_memory(self.get_iou_backbone_features(self.backbone_feat),target_box)
        ...
        max_score = torch.max(score_map).item()# * self.trans_coef
    ...
    def get_iou_features(self, backbone_feat):
        with torch.no_grad():
            if self.use_mtl:
                return self.net.bb_regressor.main_network.get_iou_feat(self.get_iou_backbone_features(backbone_feat))
            else:
    ...
    def get_iou_modulation(self, iou_backbone_feat, target_boxes):
        with torch.no_grad():
            if self.use_mtl:
                return self.net.bb_regressor.main_network.get_modulation(iou_backbone_feat, target_boxes)
            else:
    ...
    def init_memory(self, train_x: TensorList):
        ...
        #Samples for training IoU-Net
        self.iounet_samples = TensorList(
            [x.new_zeros(5, x.shape[1], x.shape[2], x.shape[3]) for x in train_x])
    ...
    def init_iou_net(self, backbone_feat):
        ...
        self.initaug_target_boxes = TensorList()
        if self.use_augs_mtl:
            for T in self.transforms:
                self.initaug_target_boxes.append(self.classifier_target_box + torch.Tensor([T.shift[1], T.shift[0], 0, 0]))

        # Get iou features
        iou_backbone_feat = self.get_iou_backbone_features(backbone_feat)
        self.initaug_iou_backbone_feat = TensorList([x for x in iou_backbone_feat])
        # Remove other augmentations such as rotation
        iou_backbone_feat = TensorList([x[:target_boxes.shape[0],...] for x in iou_backbone_feat])
        self.init_iou_backbone_feat = iou_backbone_feat
        self.iou_target_boxes = target_boxes.unsqueeze(dim=0) #(num_image,num_seq,4)->(1,1,4)
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        self.init_target_boxes_shot = target_boxes[0:-2]
    ...
    def update_iounet(self, target_box):
        # for m in self.net.bb_regressor.modules():
        #     if isinstance(m, PrRoIPool2D):
        #         m.train()

        samples_shot, target_boxes_shot = self.backbone_features_memory.get_shots()
        num_update_steps = len(samples_shot)  # Min: 1, Max: 5

        if num_update_steps > 0:
            # Inner Loop!
            update_lr = 1e-4  # As for other params of IoU-Net, it is 0.001
            losses_query = []

            proposals_shot, proposal_density_shot, gt_density_shot = prutils.sample_box_gmm(target_boxes_shot[0],
            ...
            out_mlp, bs, np =self.net.bb_regressor.main_network(self.init_iou_backbone_feat,samples_shot[0], self.iou_target_boxes,proposals_shot.unsqueeze(0).unsqueeze(0).cuda())
            iou_pred_inner = self.net.bb_regressor.iou_predictor(out_mlp).reshape(1, 1, proposals_shot.shape[0])
            loss_iou = self.iounet_loss_weight * self.iounet_objective(iou_pred_inner,sample_density=proposal_density_shot.cuda(),gt_density=gt_density_shot.cuda(), mc_dim=1)
            grad = torch.autograd.grad(loss_iou, self.net.bb_regressor.iou_predictor.parameters())

            fast_weights = list(map(lambda p: p[1] - update_lr * p[0], zip(grad,self.net.bb_regressor.iou_predictor.parameters())))  # What happen to batch norms?

            proposals_query, proposal_density_query, gt_density_query = prutils.sample_box_gmm(target_box,
                                                                                               self.proposal_params['proposal_sigma'],
                                                                                               gt_sigma=self.proposal_params['gt_sigma'],
                                                                                               num_samples=self.proposal_params['boxes_per_frame'],
                                                                                               add_mean_box=self.proposal_params.get('add_mean_box', False))

            for i in range(1, num_update_steps):
                proposals_shot, proposal_density_shot, gt_density_shot = prutils.sample_box_gmm(target_boxes_shot[i],
                                                                                                self.proposal_params['proposal_sigma'],
                                                                                                gt_sigma=self.proposal_params['gt_sigma'],
                                                                                                num_samples=self.proposal_params['boxes_per_frame'],
                                                                                                add_mean_box=self.proposal_params.get('add_mean_box',False))

                out_mlp_inner, bs, np = self.net.bb_regressor.main_network(self.init_iou_backbone_feat, samples_shot[i],self.iou_target_boxes, proposals_shot.unsqueeze(0).unsqueeze(0).cuda())
                iou_pred_inner = self.net.bb_regressor.iou_predictor(out_mlp_inner,fast_weights).reshape(1, 1, proposals_shot.shape[0])
                loss_iou = self.iounet_objective(iou_pred_inner,sample_density=proposal_density_shot.cuda(),gt_density=gt_density_shot.cuda(), mc_dim=1)
                grad = torch.autograd.grad(loss_iou,fast_weights)
                fast_weights = list(map(lambda p: p[1] - update_lr * p[0], zip(grad,fast_weights)))

            out_mlp_q, bs, np = self.net.bb_regressor.main_network(self.init_iou_backbone_feat,self.get_iou_backbone_features(self.backbone_feat), self.iou_target_boxes, proposals_query.unsqueeze(0).unsqueeze(0).cuda())
            iou_pred_q = self.net.bb_regressor.iou_predictor(out_mlp_q,fast_weights).reshape(1, 1, proposals_shot.shape[0])
            losses_query.append(self.iounet_objective(iou_pred_q,sample_density=proposal_density_query.cuda(),
                                                                                    gt_density=gt_density_query.cuda(),
                                                                                    mc_dim=1))

            self.iounet_optimizer.zero_grad()
            loss= losses_query[-1]

            loss.backward()

            self.iounet_optimizer.step()
    ...
    def update_classifier(self, train_x, target_box, learning_rate=None, scores=None):
        ...
        elif ((self.frame_num - 1) % self.params.train_skipping == 0): # or (self.flag == 'uncertain'):
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        #Save iou features for IoU-Net update->Changed!
        self.iou_features= iou_features
        ...
        self.proposals_refinement = init_boxes
        # Optimize the boxes
        ...
        self.iou_refinement = F.mse_loss(output_iou, torch.ones_like(output_iou))
    ...
    def refine_target_box_check(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        """Run the ATOM IoUNet to refine the target bounding box."""
        ...
        # Optimize the boxes
        output_boxes, output_iou = self.optimize_boxes(self.iou_features, self.proposals_refinement)
        ...
        output_boxes = output_boxes[keep_ind,:]
        output_iou = output_iou[keep_ind]
        self.iou_refinement_check = F.mse_loss(output_iou, torch.ones_like(output_iou))
