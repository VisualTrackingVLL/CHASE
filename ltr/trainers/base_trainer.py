...
class BaseTrainer:
    ...
    def __init__(self, actor, loaders, optimizer, settings, lr_scheduler=None,momentum_args=0.9, weight_decay_args=3e-4,arch_learning_rate=3e-4, arch_weight_decay=1e-3,search=False, hold_out=None, num_epochs=50):
        ...
        if search:
            self.optimizer,self.dummy_optimizer = optimizer
            self.lr_scheduler,self.dummy_lr_scheduler = lr_scheduler
        else:
            self.optimizer = optimizer
            self.lr_scheduler = lr_scheduler
            self.dummy_lr_scheduler= None
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if self.dummy_lr_scheduler is not None:
                        self.dummy_lr_scheduler.step()
                        # elif self.method=='PR-DARTS':
