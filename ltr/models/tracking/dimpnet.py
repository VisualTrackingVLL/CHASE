...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, neck, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        super().__init__()
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        train_outNAS=self.neck(train_feat)
        test_outNAS=self.neck(test_feat)

        # Run classifier module
        target_scores = self.classifier(train_outNAS, test_outNAS, train_bb, *args, **kwargs)
        target_scores_min, target_scores_max = torch.min(target_scores[0]), torch.max(target_scores[0])
        ...
        return target_scores, iou_pred, None

    def get_backbone_clf_feat(self, backbone_feat):
        self.classification_layer=['layer2','layer3']
    ...
    def extract_classification_feat(self, backbone_feat): #change
        return self.classifier.extract_classification_feat(self.neck(self.get_backbone_clf_feat(backbone_feat)))
...
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
              ...
              clf_feat_norm=True, init_filter_norm=False, final_conv=True,
              out_feature_dim=2048, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
              mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
              score_act='relu', act_param=None, target_mask_act='sigmoid',
              detach_length=float('Inf'), frozen_backbone_layers=(), search=False):

    # Backbone
    backbone_net = backbones.resnet50(pretrained=backbone_pretrained, frozen_layers=frozen_backbone_layers)

    # Feature normalization
    norm_scale = math.sqrt(1.0 / (out_feature_dim * filter_size * filter_size)) #16 or 1 -> set to 1 similar to dimp
    ...
    clf_NAS=clf_features.NAS(search=search)
    #clf_NAS=clf_features.PRDARTS(search=search)
    clf_feature_extractor = clf_features.residual_bottleneck(feature_dim=4*256,
    ...
    # DiMP network
    net = DiMPnet(feature_extractor=backbone_net, neck=clf_NAS,classifier=classifier, bb_regressor=bb_regressor,
                  classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net
...
def klcedimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
                  ...
                  detach_length=float('Inf'), alpha_eps=0.0, train_feature_extractor=True,
                  init_uni_weight=None, optim_min_reg=1e-3, init_initializer='default', normalize_label=False,
                  label_shrink=0, softmax_reg=None, label_threshold=0, final_relu=False, frozen_backbone_layers=(),search=False):
