...
class Alpha_Weights(nn.Module):
  def __init__(self, channels,batch_size):
    super(Alpha_Weights, self).__init__()
    self.channels = channels
    self.batch_size= batch_size
    self.alpha = Parameter(torch.ones(1,channels,1,1))
    self.alpha.requires_grad = True

  def forward(self, cor_map):
    #return (cor_map * self.alpha.expand_as(cor_map)).sum(dim=2)
    return (cor_map * self.alpha).sum(dim=2)
...
class LinearFilter(nn.Module):
    ...
    def __init__(self, filter_size, filter_initializer, filter_optimizer=None, feature_extractor=None,C_curr=256,batch_size=4):
        super().__init__()
        ...
        self.alpha = Alpha_Weights(channels=4 * C_curr, batch_size=batch_size)
        # self.last_conv = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=2, padding=1,
        #                            dilation=2,
        #                            stride=1)
        # # nn.Conv2d(in_channels=4*C_curr, out_channels=4*C_curr, kernel_size=3, padding=0)
        # self.last_bn = nn.BatchNorm2d(1, eps=0.001)
        ...
        for m in self.feature_extractor.modules():
            if isinstance(m, nn.Conv2d):
                ...
                if m.affine==True: #DARTS and Dimp should be different
    ...
    def forward(self, train_feat, test_feat, train_bb, *args, **kwargs):
        ...
        train_feat_min, train_feat_max = torch.min(train_feat), torch.max(train_feat)
        test_feat_min, test_feat_max = torch.min(test_feat), torch.max(test_feat)
        # Extract features
        ...
        train_feat_min1, train_feat_max1 = torch.min(train_feat), torch.max(train_feat)
        test_feat_min1, test_feat_max1 = torch.min(test_feat), torch.max(test_feat)

        # Train filter
        filter, filter_iter, losses = self.get_filter(train_feat, train_bb, *args, **kwargs)

        # Classify samples using all return filters
        #test_scores = [self.classify_depthwise(f, test_feat) for f in filter_iter]
    ...
    def classify(self, weights, feat):
        ...
        #mod_scores=self.last_bn(self.last_conv(scores.view(30,19,19))).view(3,10,19,19)

        return scores

    def classify_depthwise(self, weights, feat):
        """Run classifier (filter) on the features (feat)."""

        scores = self.alpha(filter_layer.apply_filter_depthwise(feat, weights))
        #mod_scores=self.last_bn(self.last_conv(scores.view(30,19,19))).view(3,10,19,19)
