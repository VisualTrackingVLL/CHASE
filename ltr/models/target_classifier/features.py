...
import ltr.cnn.genotypes as genotypes
from ltr.cnn.model_search import Network_Search
...
def residual_bottleneck(feature_dim=4*256, num_blocks=1, l2norm=True, final_conv=False, norm_scale=1.0, out_dim=None,
...
def NAS(search=False):
    ...
    if search == True:
        ...
        model = Network_Search(C=256)  # We may can ignore C!!!??? Check it later
        model = model.cuda()
    return model
