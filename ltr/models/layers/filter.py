...
def apply_filter_depthwise(feat, filter, dilation_factors=None):
    """Applies the filter on the input features (feat). The number of groups is automatically calculated.
    args:
        feat: These are the input features. Must have dimensions (images_in_sequence, sequences, feat_dim, H, W)
        filter: The filter to apply. Must have dimensions (sequences, feat_dim, fH, fW) or (sequences, filters, feat_dim/groups, fH, fW)
    ...
    assert num_filters % groups == 0 and num_channels % groups == 0
    feat_reshaped = feat.reshape(num_images, num_sequences * num_channels, feat.shape[-2], feat.shape[-1])
    filter_reshaped = filter.reshape(num_sequences * num_channels, 1, filter.shape[-2], filter.shape[-1])
    #for i in range(num_images):
    scores=F.conv2d(feat_reshaped, filter_reshaped,padding=padding, groups=num_sequences * num_channels)
    #np_scores=torch.stack(scores,0)
    #return np_scores.view(num_images, num_sequences, num_channels, np_scores.shape[-2], np_scores.shape[-1]).cuda()
    return scores.view(num_images, num_sequences, num_channels, scores.shape[-2], scores.shape[-1]).cuda()
